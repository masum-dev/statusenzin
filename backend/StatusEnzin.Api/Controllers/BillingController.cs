...
public class BillingController : ControllerBase
{
    ...
    public async Task<IActionResult> GetSubscription()
    {
        ...
        var monitorCount = await _db.Monitors.CountAsync(m => m.TenantId == tenantId.Value);
        var statusPageCount = await _db.StatusPages.CountAsync(sp => sp.TenantId == tenantId.Value);

        int monitorLimit = PlanPricing.GetMaxMonitors(plan);
        int statusPageLimit = PlanPricing.GetMaxStatusPages(plan);
