...
public class MonitorsController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateMonitor([FromBody] CreateMonitorRequest req)
    {
        ...
        var currentCount = await _db.Monitors.CountAsync(m => m.TenantId == tenantId.Value);

        var plan = tenant?.PlanType ?? "Starter";
        int maxMonitors = PlanPricing.GetMaxMonitors(plan);

        if (currentCount >= maxMonitors)
        {
            return BadRequest(new { message = $"Plan limit reached. Your current plan ({plan}) allows up to {maxMonitors} monitors." });
        }

        var interval = req.CheckIntervalSeconds > 0 ? req.CheckIntervalSeconds : 300;
        if (!PlanPricing.IsIntervalAllowed(plan, interval))
        {
            int minInterval = PlanPricing.GetMinIntervalSeconds(plan);
            return BadRequest(new { message = $"Your {plan} plan does not support a {interval}-second check interval. The minimum allowed interval for your plan is {minInterval} seconds. Upgrade your plan to use faster monitoring." });
        }
    ...
    public async Task<IActionResult> UpdateMonitor(Guid id, [FromBody] UpdateMonitorRequest req)
    {
        ...
        var tenant = await _db.Tenants.IgnoreQueryFilters().FirstOrDefaultAsync(t => t.Id == monitor.TenantId);
        var plan = tenant?.PlanType ?? "Starter";

        if (!PlanPricing.IsIntervalAllowed(plan, req.CheckIntervalSeconds))
        {
            int minInterval = PlanPricing.GetMinIntervalSeconds(plan);
            return BadRequest(new { message = $"Your {plan} plan does not support a {req.CheckIntervalSeconds}-second check interval. The minimum allowed interval for your plan is {minInterval} seconds. Upgrade your plan to use faster monitoring." });
        }
