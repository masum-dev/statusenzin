...
export default function DashboardOverview() {
  ...
  const INTERVAL_OPTIONS = [
    { value: 300, label: '5 minutes', minPlan: 'Starter' },
    { value: 60, label: '1 minute', minPlan: 'Pro' },
    { value: 30, label: '30 seconds', minPlan: 'Business' },
  ];
  const isIntervalAllowed = (plan: string, interval: number) => {
    const opt = INTERVAL_OPTIONS.find(o => o.value === interval);
    const minRank = PLAN_RANK[opt?.minPlan ?? 'Starter'] ?? 0;
  ...
  useEffect(() => {
    ...
    api.get('/billing/subscription')
      .then((res) => setCurrentPlan(res.data?.planType || 'Starter'))
      .catch(() => {});
  }, []);
                    ...
                    >
                      {INTERVAL_OPTIONS.map((opt) => (
                        <option
                          key={opt.value}
                          value={opt.value}
                          disabled={!isIntervalAllowed(currentPlan, opt.value)}
                        >
                          {opt.label}{opt.minPlan !== 'Starter' ? ` (${opt.minPlan}+)` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
